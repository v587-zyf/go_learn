...
func CreateIndex(indexName string) {
	...
	_, err := GetEsClient().CreateIndex(indexName).BodyJson(mapping).Do(context.Background())
	if err != nil {
...
func CheckIndexExists(indexName string) bool {
	isHas, err := GetEsClient().IndexExists(indexName).Do(context.Background())
	if err != nil {
...
func UpdateIndex(indexName string) {
	...
	_, err := GetEsClient().PutMapping().Index(indexName).BodyJson(mapping).Do(context.Background())
	if err != nil {
...
func DelIndex(indexName string) {
	_, err := GetEsClient().DeleteIndex(indexName).Do(context.Background())
	if err != nil {
