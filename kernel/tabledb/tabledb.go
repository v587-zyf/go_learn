...
func (t *TableDb) loadFile(filename string, sheetInfos []SheetInfo) error {
	...
	for _, sheetInfo := range sheetInfos {
		...
		// read from 3rd line,2nd row
		objs, err := ReadXlsxSheet(sheet, objPropType, 2, 1, nil)
		if err != nil {
			return err
		}
...
func InitError(code int32, message string, lang enums.LANGUAGE) errcode.ErrCode {
	e := errcode.CreateErrCode(code, message, lang)
